...
interface PlayerSetupProps {
  onStartGame: (players: string[], isPermanentGame: boolean, gameId?: number) => void
}

export const PlayerSetup = ({ onStartGame }: PlayerSetupProps) => {
  const [manualPlayers, setManualPlayers] = useState(['', ''])
  const [permanentPlayers, setPermanentPlayers] = useState<Player[]>([])
  const [selectedPermanentPlayers, setSelectedPermanentPlayers] = useState<Player[]>([])
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-xl shadow-lg p-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">Game Type</h2>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <button
          ...
          disabled={
            (gameMode === 'permanent' && selectedPermanentPlayers.length < 2) ||
            (gameMode === 'manual' &&
        ...
        >
          {gameMode === 'permanent' ? 'Start Tracked Game' : 'Start Quick Game'}
        </button>
      </div>
    </div>
  )
